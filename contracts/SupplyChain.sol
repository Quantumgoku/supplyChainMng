...
contract SupplyChain {
    ...
    //events to show the blocks mined
    event RawMaterialSupplied(address from, uint256 medicineID, uint256 rmsID, string message);
    event MedicineDistributed(address from, uint256 medicineID, uint256 disID, string message);
    event MedicineManufactured(address from, uint256 medicineID, uint256 manID, string message);
    event MedicineRetailed(address from, uint256 medicineID, uint256 retID, string message);
    event MedicineSold(address from, uint256 medicineID, uint256 retID, string message);
    event MedicineAdded(address addedBy, uint256 medicineID, string name, string message);
    ...
    {
        require(medicineCtr > 0, "Medicine not available in stock");
    ...
    function RMSsupply(uint256 _medicineID) public {
        ...
        require(MedicineStock[_medicineID].stage == STAGE.Init, "Medicine not in initial stage");
        MedicineStock[_medicineID].RMSid = _id;
        MedicineStock[_medicineID].stage = STAGE.RawMaterialSupply;

        emit RawMaterialSupplied(msg.sender, _medicineID, _id, "Raw Material Supply Stage");
        emit LogStageChange(msg.sender, _medicineID, "RMS Supply", "Raw Material Supply Stage");
    }
    ...
    function Manufacturing(uint256 _medicineID) public {
        ...
        require(MedicineStock[_medicineID].stage == STAGE.RawMaterialSupply , "Medicine not in raw material supply stage");
        MedicineStock[_medicineID].MANid = _id;
        MedicineStock[_medicineID].stage = STAGE.Manufacture;

        emit MedicineManufactured(msg.sender , _medicineID, _id, "Manufacturing Stage");
        emit LogStageChange(msg.sender, _medicineID, "Manufacturing", "Manufacturing Stage");
    }
    ...
    function Distribute(uint256 _medicineID) public {
        ...
        emit MedicineDistributed(msg.sender, _medicineID, _id, "Distribution Stage");
        emit LogStageChange(msg.sender, _medicineID, "Distribution", "Distribution Stage");
    }
    ...
    function Retail(uint256 _medicineID) public {
        ...
        emit MedicineRetailed(msg.sender, _medicineID, _id, "Retail Stage");
        emit LogStageChange(msg.sender, _medicineID, "Retail", "Retail Stage");
    }
    ...
    function sold(uint256 _medicineID) public {
        ...
        emit MedicineSold(msg.sender, _medicineID, _id, "Medicine Sold");
        emit LogStageChange(msg.sender, _medicineID, "Sold", "Medicine Sold");
    }
